...
#ifndef SEQ64_MUTEX_MINGW
#define SEQ64_MUTEX_MINGW
...
protected:
    ...
    void checkSetOwnerAfterLock()
    {
        ...
        if (mOwnerThread == self)
        {
            ...
            (
                stderr,
                "FATAL: Recursive locking or non-recursive mutex detected. "
    ...
    void checkSetOwnerBeforeUnlock ()
    {
        ...
        if (mOwnerThread != self)
        {
            ...
            (
                stderr,
                "FATAL: Recursive unlocking of non-recursive mutex detected. "
                "Throwing system exception\n"
            );
            fflush(stderr);
            throw system_error(EDEADLK, generic_category());
...
// You can use the scoped locks and other helpers that are still provided by
// <mutex> In that case, you must include <mutex> before inclusing this file,
// so that this file will not try to redefine them
